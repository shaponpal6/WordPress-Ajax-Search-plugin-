    ...
    function master(api, feature,  target) {
        ...
        // once(config)
            .then(function (response) {
                ...
                var response = common.jsonParse(response.responseText, 'FuesSearch');
...
// select templete
    function masterChoose(label, lab, target) {
        ...
        target.innerHTML = '';
        Promise.resolve(template.set('products', filterResult, target));
    ...
    function run(action, val, target) {
        var pipe2 = new Promise(function(resolve, reject) {
            resolve(function () {
                // Set Config
                var ais = common.aisBot(target);
        ...
        pipe2.then(function () {
            if (action === 'master') {
                ...
                console.log('_______ Action ____not ok____', action);
            }
