let controller = (function () {
    ...
    function singleAction(val, target) {
        ...
        if (fe.symbols.indexOf(val.substr(0, 1)) !== -1) {
            actionBot.bot(val.substr(0, 1), val , 0, target);
            return true;
        } else if (fe.action.indexOf(val.split(' ')[0]) !== -1) {
            actionBot.bot(val.split('')[0], val, 1, target);
            return true;
        } else if (fe.toWords.indexOf(val.split(' ').slice(0,2).join(' ')) !== -1) {
            actionBot.bot(val.split(' ').slice(0,2).join(' '), val, 2, target);
            return true;
        }
    ...
    function quickAction(val,target) {
        ...
        if (fe.action.indexOf(first) !== -1) {
            actionBot.bot(first, val,6,target);
            return true;
        }
    ...
    function qknowledge(val,target) {
        async function knowledge() {
            return await new Promise(res => res(knowledgeBase.bot(val,target)));
        }
        knowledge().then((r)=>{
            return true;
        }).catch(e=>{
